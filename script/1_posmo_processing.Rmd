...
# Visualisation of all labelized tmode datasets
posmo_walks <- posmo %>% filter(tmode_manual == "walk") %>% filter(region == "Wallis") #Waadt #just for visualization, 
...
# to see segments
posmo_test <- posmo |>
  filter(posmo$datetime > as.POSIXct("2023-04-15 09:18:10", tz = "Europe/Zurich") & posmo$datetime < as.POSIXct("2023-04-15 9:24:30", tz = "Europe/Zurich"))
posmo_test
...
posmo_filter <- posmo %>% filter(tmode_manual == "ski") %>% filter(as.Date(datetime) == "2023-04-13") 
