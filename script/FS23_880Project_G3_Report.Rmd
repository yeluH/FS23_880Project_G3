...
mvmt_data_filtered %>%
  #loop
    #sum the timelag
    #when timelag > 100 secs, increment index
timelag_sum = 0
for (i in 2:nrow(mvmt_data_filtered)) {
  timelag_i = timelag
  if (mvmt_data_segmentized$gap[i-1] == TRUE) {
    mvmt_data_segmentized$segment_ID_gap[i] <- mvmt_data_segmentized$segment_ID_gap[i-1] + 1
  } else {
...
7) Compute trajectories derivatives for each segment
...
mvmt_all <- merge(st_drop_geometry(mvmt_seg_summary),
                  ...
                  by = c('segment_ID_both', 'source', 'tmode_manual', 'duration_secs', 'static',
                         'static_percent'))
## 1459 RECORDS


# Save the file from steps above
write.csv(select(mvmt_all, -c("lst_geometry")), 
...
split <- sample.split(mvmt_all, SplitRatio = 0.2)
...
set.seed(100)
mvmt_all <- mvmt_all %>% filter(label != "plane")
...
mvmt_all_train_rf_chunk <- mvmt_all_train
mvmt_all_train_rf_chunk$chunk <- chunk
mvmt_all_train_rf_chunk$label <- as.factor(mvmt_all_train_rf_chunk$label)
...
for(m in 1:3){ # Loop through the different models
  ...
  for(i in 1:k_fold){
    set.seed(21)
    rf <- randomForest(x = dplyr::select(mvmt_all_train_rf_chunk[chunk != i, ], -label, -chunk), 
                     y = mvmt_all_train_rf_chunk[chunk != i, ]$label, 
