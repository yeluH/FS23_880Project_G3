...
mvmt_data_filter_gap <- mvmt_data_gap |> filter(gap)
mvmt_data_filter_non <- mvmt_data_gap |> filter(!gap)

mvmt_data_filter_gap_wgs <- mvmt_data_filter_gap %>% st_transform(crs = 4326)
mvmt_data_filter_non_wgs <- mvmt_data_filter_non%>% st_transform(crs = 4326)

m <- leaflet() %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers(data = mvmt_data_filter_non_wgs,
                   radius = 0.2,
                   popup = paste(
                     " steplength: ", round(mvmt_data_filter_non_wgs$steplength_m,2),
                     " timelag: ", mvmt_data_filter_non_wgs$timelag,
                     " speed_ms: ", round(mvmt_data_filter_non_wgs$speed_ms,2),
                     " source: ", mvmt_data_filter_non_wgs$source
                     ),
                   color = 'blue') %>%
  addCircleMarkers(data = mvmt_data_filter_gap_wgs,
                   radius = 0.2,
                   popup = paste(
                     " steplength: ", round(mvmt_data_filter_gap_wgs$steplength_m,2),
                     " timelag: ", mvmt_data_filter_gap_wgs$timelag,
                     " speed_ms: ", round(mvmt_data_filter_gap_wgs$speed_ms,2),
                     " source: ", mvmt_data_filter_gap_wgs$source
                     ),
                   color = 'red') %>%
  addLegend(position = 'topright', 
            colors = c('blue', 'red'), 
...
mvmt_data_segmentized <- mvmt_data_segmentized %>%
  ...
  mutate(duration_mins = as.integer(difftime(max(datetime), min(datetime), units = "mins")))
...
mvmt_data_filter_static <- mvmt_data_add_static |> filter(static)
mvmt_data_filter_non <- mvmt_data_add_static |> filter(!static)

mvmt_data_filter_static_wgs <- mvmt_data_filter_static %>% st_transform(crs = 4326)
mvmt_data_filter_non_wgs <- mvmt_data_filter_non%>% st_transform(crs = 4326)
...
m <- leaflet() %>%
  ...
  addCircleMarkers(data = mvmt_data_filter_non_wgs,
                   ...
                   radius = 0.2,
                   popup = paste(
                     "step_mean: ", mvmt_data_filter_non_wgs$stepMean_midw,
                     " steplength: ", round(mvmt_data_filter_non_wgs$steplength_m,2),
                     " timelag: ", mvmt_data_filter_non_wgs$timelag,
                     " speed_ms: ", round(mvmt_data_filter_non_wgs$speed_ms,2),
                     " source: ", mvmt_data_filter_non_wgs$source
                   ...
                   color = 'blue') %>%
  addCircleMarkers(data = mvmt_data_filter_static_wgs,
                   #opacity = 0.3,
                   radius = 0.2,
                   popup = paste(
                     "step_mean: ", mvmt_data_filter_static_wgs$stepMean_midw,
                     " steplength: ", round(mvmt_data_filter_static_wgs$steplength_m,2),
                     " timelag: ", mvmt_data_filter_static_wgs$timelag,
                     " speed_ms: ", round(mvmt_data_filter_static_wgs$speed_ms,2),
                     " source: ", mvmt_data_filter_static_wgs$source
                     ),
                   color = 'red') %>%
  addLegend(position = 'topright', 
            colors = c('blue', 'red'), 
...
mvmt_data_filtered <- mvmt_data_add_static %>%
  group_by(segment_ID_both, source, tmode_manual, duration_mins, static) %>%
...
```{r}
# SUMMARIZE
mvmt_data_summarized <- mvmt_data_filtered %>%
  summarize(
...
extract <- mvmt_data_filtered %>% filter(tmode_manual == "train")
...
#filter extract
lst_extract <- c("1-60-1", #huge gap to filter out.
...
filter_true <- extract_f |> filter(isClosetoRails) %>% st_transform(crs = 4326)
filter_false <- extract_f |> filter(!isClosetoRails) %>% st_transform(crs = 4326)
...
m <- leaflet() %>%
  ...
  addCircleMarkers(data = filter_false,
                   radius = 0.2,
                   popup = paste(
                     "tmode: ", filter_false$tmode_manual, ". ID: ", filter_false$segment_ID_both
                     ),
                   color = 'red') %>%
  addCircleMarkers(data = filter_true,
                   radius = 0.2,
                   popup = paste(
                     "tmode: ",  filter_true$tmode_manual, ". ID: ", filter_false$segment_ID_both
                     ),
                   color = 'blue') %>%
  addLegend(position = 'topright', 
            colors = c('blue', 'red'), 
