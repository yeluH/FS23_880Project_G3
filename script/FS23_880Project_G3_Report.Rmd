...
sabi_segmentized <- sabi %>%
  mutate(
    ...
    segment_ID_both = as.numeric(segment_ID_user) * as.numeric(segment_ID_static) * as.numeric(segment_ID_tmode),
  )


#unique(sabi$segment_ID_both) %>% length()

#sabi %>% dplyr::select(c("segment_ID_static", "segment_ID_tmode", "segment_ID_user", "source", "segment_ID_both")) %>% st_drop_geometry() %>% unique() %>% View()
