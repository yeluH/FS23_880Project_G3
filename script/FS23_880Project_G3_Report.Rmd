...
sabi_filter_gap <- sabi_gap |> filter(gap)
sabi_filter_non <- sabi_gap |> filter(!gap)

sabi_filter_gap_wgs <- sabi_filter_gap %>% st_transform(crs = 4326)
...
m <- leaflet() %>%
  ...
  addCircleMarkers(data = sabi_filter_non_wgs,
                   ...
                   color = 'blue') %>%
  addCircleMarkers(data = sabi_filter_gap_wgs,
                   radius = 0.2,
                   popup = paste(
                     " steplength: ", round(sabi_filter_gap_wgs$steplength_m,2),
                     " timelag: ", sabi_filter_gap_wgs$timelag,
                     " speed_ms: ", round(sabi_filter_gap_wgs$speed_ms,2),
                     " source: ", sabi_filter_gap_wgs$source
                     ),
                   color = 'red') %>%
  addLegend(position = 'topright', 
            colors = c('blue', 'red'), 
...
2) Segmentation per user and transport mode  
...
# MAP VISUALISATION
n_segs <- sabi_segmentized$segment_ID_both %>% unique() %>% length()
sabi_wgs <- sabi_segmentized %>% st_transform(crs = 4326)
...
sabi_add_static <- sabi_segmentized |>
  group_by(segment_ID_both) %>%
  #ungroup() |>
  mutate(
...
#sabi %>% group_by(source) %>% summarize(mean = mean(stepMean)) %>% View()




#Visualisation of static / non static:
sabi_filter_static <- sabi_add_static |> filter(static)
sabi_filter_non <- sabi_add_static |> filter(!static)
...
#sabi_add_static %>% dplyr::select(c("segment_ID_static", "segment_ID_tmode", "segment_ID_user", "source", "segment_ID_gap", "segment_ID_both")) %>% st_drop_geometry() %>% unique() %>% View()

#sabi %>% dplyr::select(c("segment_ID_static", "segment_ID_tmode")) %>% st_drop_geometry() %>% unique() %>% View()


# filter static and small segments
sabi_filtered <- sabi_add_static %>%
...
# MAP VISUALISATION
n_segs <- sabi_filtered$segment_ID_both %>% unique() %>% length()
sabi_wgs <- sabi_filtered %>% st_transform(crs = 4326)
...
```{r}
# SUMMARIZE
sabi_summarized <- sabi_filtered %>%
  summarize(
    stepMean_midw_mean = mean(stepMean_midw), 
    steplength_mean = mean(steplength_m), 
    speed_ms_mean = mean(speed_ms), 
    lst_geometry = list(geometry))
sabi_summarized %>% View()
...
-> add it in sabi_summarized

Import the railways:  
```{r}
#mvmt_data_sfc <- mvmt_data %>% st_bbox() %>% st_as_sfc()
#extract <- mvmt_data[1:100,]
extract <- mvmt_data %>% filter(tmode_manual == "train")
rails_shp <- read_sf(here(data_folder, "railways", "rails_train_dissolved.shp")) %>% st_zm() 

joined_data <- extract %>% 
  st_join(rails_shp, join = st_is_within_distance, 20)

extract$isClosetoRails <- !is.na(joined_data$xtf_id)




#Visualisation of proximity:
filter_true <- extract |> filter(isClosetoRails) %>% st_transform(crs = 4326)
filter_false <- extract |> filter(!isClosetoRails) %>% st_transform(crs = 4326)

m <- leaflet() %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers(data = filter_false,
                   radius = 0.2,
                   popup = paste(
                     "tmode: ", filter_false$tmode_manual
                     ),
                   color = 'red') %>%
  addCircleMarkers(data = filter_true,
                   radius = 0.2,
                   popup = paste(
                     "tmode: ", filter_true$tmode_manual
                     ),
                   color = 'blue') %>%
  addLegend(position = 'topright', 
            colors = c('blue', 'red'), 
