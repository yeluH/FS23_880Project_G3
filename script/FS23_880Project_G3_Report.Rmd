...
check_pkg("caret") #for statistics
...
The least performant tmode (kick-scooter, cable_car, boat, t_bar) are the one with the least representation in the test  set (prevalence), and the best have the most (train, tram, walk, car).

```{r, echo = TRUE, warning = FALSE, message = FALSE}
# Confusion matrix
caret::confusionMatrix(knn_test_pred1, mvmt_all_test$label)
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
caret::confusionMatrix(rf_test_pred1, mvmt_all_test$label)
```
...
-   data attributes available ( for ex. no height for planes) ...
...
-   SwissTLM3D: ... source: map.geoadmin.ch : Arrêts des transports publics (Office fédéral des transports OFT)
