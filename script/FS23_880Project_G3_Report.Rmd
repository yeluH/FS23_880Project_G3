...
(to check distance to be close, 60% ??)
```{r}
...
#Visualisation of proximity:
visu_true <- mvmt_seg_final |> filter(isClosetoCables) %>% st_transform(crs = 4326)
visu_false <- mvmt_seg_final |> filter(!isClosetoCables) %>% st_transform(crs = 4326)
...
mvmt_seg_final_summarized <- mvmt_seg_final %>%
  summarize(
    ...
    isClosetoCables =  (sum(isClosetoCables) / n() * 100 ) > 70.0
    )
...
extract <- mvmt_seg_final %>% filter(tmode_manual == "bus")
