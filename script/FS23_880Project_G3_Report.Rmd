...
posmo <- read_delim(here(data_folder, "posmo_2023-06-14_2.csv"), ",",show_col_types = FALSE) %>% st_as_sf(coords = c("lon_x", "lat_y"), crs=4326, remove = FALSE) |> st_transform(2056)
...
condition_tram <- 
  ...
  gps_data$datetime >= as.POSIXct("2023-05-29 17:40:10") & gps_data$datetime <= as.POSIXct("2023-05-29 17:59:50") | 
  gps_data$datetime >= as.POSIXct("2023-05-29 18:20:00") & gps_data$datetime <= as.POSIXct("2023-05-29 18:27:35") | 
...
condition_walk <- 
  ...
  gps_data$datetime >= as.POSIXct("2023-04-30 18:20:25") & gps_data$datetime <= as.POSIXct("2023-04-30 19:38:55") |
  gps_data$datetime >= as.POSIXct("2023-06-02 13:22:45") & gps_data$datetime <= as.POSIXct("2023-06-02 13:28:00") |
  gps_data$datetime >= as.POSIXct("2023-06-05 14:49:00") & gps_data$datetime <= as.POSIXct("2023-06-05 14:57:00") |
...
-> PROBABLY THIS SECTION TO DROP.  
Because we will do it per segment, and not for individuals.  
```{r}
#compute new values
...
sabi_filter_static <- sabi_gap |> filter(gap)
sabi_filter_non <- sabi_gap |> filter(!gap)
...
# Add user id so we are also sure that the segments are separated by user
#TODO: maybe multiplication is not the best way to build a unique id.

sabi_segmentized <- sabi %>%
  mutate(
    ...
    segment_ID_both = as.numeric(segment_ID_user) * as.numeric(segment_ID_static) * as.numeric(segment_ID_gap) * as.numeric(segment_ID_tmode)
  )


#unique(sabi$segment_ID_both) %>% length()

sabi_segmentized %>% dplyr::select(c("segment_ID_static", "segment_ID_tmode", "segment_ID_user", "source", "segment_ID_gap", "segment_ID_both")) %>% st_drop_geometry() %>% unique() %>% View()

#sabi %>% dplyr::select(c("segment_ID_static", "segment_ID_tmode")) %>% st_drop_geometry() %>% unique() %>% View()

# Add duration per segment
sabi_segmentized <- sabi_segmentized %>%
  group_by(segment_ID_both) %>%
  mutate(duration_min = as.integer(difftime(max(datetime), min(datetime), "mins")))


# GROUP BY SEGMENTS
sabi_grouped <- sabi_segmentized %>%
  group_by(segment_ID_both, source, tmode_manual, duration_min, region, static) %>%
  # remove static segments
  filter(static == FALSE) %>%
  #remove the too small segments (duration < 1 min?)
  filter(duration_min >= 1)

# # SUMMARIZE
# sabi_summarized <- sabi_grouped %>%
#   summarize(step_mean = mean(steplength_m), speed_ms_mean = mean(speed_ms) ,lst_geometry = list(geometry))
# 
# # we should use roll mean ? (zoo library) (ex.2 task5)
# check_pkg("zoo")
# 
# sabi_grouped %>%
#   group_by(segment_ID_both) %>% 
#   mutate(
#     mean_speed = rollmean(speed_ms, k=3, fill = NA, align = "left")
#   ) %>% View()


# MAP VISUALISATION
n_segs <- sabi_grouped$segment_ID_both %>% unique() %>% length()
sabi_wgs <- sabi_grouped %>% st_transform(crs = 4326)
...
TODO: create nice linestring for the grouped segments
<!-- ```{r} -->
<!-- # GROUP BY SEGMENTS -->
<!-- #TODO: create nice linestring -->

<!-- #to see how many points in a linestring: -->
<!-- test2$geometry[12] %>% npts() -->
<!-- test2filter <- test2 %>% filter(npts(geometry) > 1) -->
<!-- ggplot() + geom_sf(data=test2, aes(color=as.factor(segment_ID_both))) -->
...
<!-- # GROUP BY SEGMENT -->
<!-- test <- sabi %>% -->
<!--   group_by(segment_ID_both, source, tmode_manual, region, static) %>%  -->
<!--   summarize(step_mean = mean(steplength_m), speed_ms_mean = mean(speed_ms) ,lst_geometry = list(geometry)) %>% -->
<!--   filter(st_geometry_type(geometry) == "MULTIPOINT") %>% -->
<!--   mutate(geometry = sf::st_cast(geometry, "LINESTRING")) -->

<!-- test %>% View() -->

<!-- #st_geometry_type(test$geometry) == "MULTIPOINT" -->
<!-- #  -->
<!-- # mutate( -->
<!-- #     geometry = case_when( -->
<!-- #       st_geometry_type(geometry) == "MULTIPOINT" ~ sf::st_cast(geometry, "LINESTRING"), -->
<!-- #       st_geometry_type(geometry) == "POINT" ~ NA #we should drop the points bc too small anyway -->
<!-- #     ) -->
<!-- #   ) %>% -->


<!-- # VISUALISATION -->
<!-- n_segs <- test$segment_ID_both %>% unique() %>% length() -->
<!-- test_wgs <- test %>% st_transform(crs = 4326) -->

<!-- set.seed(2) -->
<!-- factpal <- colorFactor(sample(rainbow(n_segs)), domain =test_wgs$segment_ID_both) -->

<!-- m <- leaflet() %>% -->
<!--   #addTiles() %>%  # Add default OpenStreetMap map tiles -->
<!--   addProviderTiles("Esri.WorldGrayCanvas") %>% -->
<!--   addPolylines(data = test_wgs, -->
<!--                    #opacity = 0.3, -->
<!--                    #radius = 0.2, -->
<!--                    popup = paste("seg_ID: ", test_wgs$segment_ID_both), -->
<!--                    color = ~factpal(segment_ID_both)) %>% -->
<!--   addLegend(position = 'topright',  -->
<!--             #colors = c('red'),  -->
<!--             #labels = c('static'), -->
<!--             pal = factpal, -->
<!--             values = test_wgs$segment_ID_both, -->
<!--             title = 'Grouped by segments id') -->
...
mvmt_data_filter <- mvmt_data %>% filter(tmode_manual == "boat") #%>% filter(!is.na(tmode_manual)) #%>% filter(region == "Zurich") 
