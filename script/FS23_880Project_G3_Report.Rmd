...
## Load the data
gps_data_raw <- read_delim("D:/GEO880/Project/yelu_dataset_0609.csv", ";")
...
gps_data <- select(gps_data, Date, Time, Latitude, Longitude, Altitude, geometry)

gps_data$Datetime <- as.POSIXct(paste(as.Date(gps_data$Date, format = '%d.%m.%Y'), data$Time), format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
gps_data <- gps_data %>% mutate(Datetime = Datetime %>% with_tz(tzone = "Europe/Zurich"))

gps_data$source <- "gps"
gps_data <- select(gps_data, source, Datetime, Latitude, Longitude, Altitude, geometry)
...
gps_data$travelmode <- NA


# Conditions for different travel modes
condition_bus <- gps_data$Datetime >= as.POSIXct("2023-04-21 16:18:35") &  gps_data$Datetime <= as.POSIXct("2023-04-21 16:23:40")
...
condition_walk <- 
  ...
  gps_data$Datetime >= as.POSIXct("2023-04-25 15:37:00") & gps_dataDatetime <= as.POSIXct("2023-04-25 15:40:15")
...
col_order <- c("source", "datetime", "weekday", "tmode_posmo", "tmode_manual" ,"E", "N", "H", "geometry")
...
filter <- mvmt_data |>
    filter(as.Date(datetime) == "2023-04-12")
filter

sabi <- filter |>
  ungroup() |>
  #mutate(static = timelag >20)
...
sabi_filter_static <- sabi |>
    filter(static)

sabi_filter_non <- sabi |>
    filter(!static)
